...
{
	cgroup_limits_t *limits = NULL, *root_limits = NULL;
	...
	limits->allow_mems = xstrdup(root_limits->allow_mems);

	/* User constrain */
	limits->allow_cores = xstrdup(job_alloc_cpus);
	xstrfmtcat(limits->allow_cores, ",%s", root_limits->allow_cores);
	...
	/* Step constrain */
	limits->allow_cores = xstrdup(step_alloc_cpus);
